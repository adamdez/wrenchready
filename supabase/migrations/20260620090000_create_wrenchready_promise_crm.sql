-- WrenchReady Promise CRM core tables.
-- Safe to run on a fresh project. For an existing production table with drift,
-- review columns and constraints before applying.
...
alter table public.wrenchready_inbound
...
create index if not exists wrenchready_inbound_updated_at_idx
  on public.wrenchready_inbound (updated_at desc);
...
create index if not exists wrenchready_promise_updated_at_idx
  on public.wrenchready_promise (updated_at desc);
...
create or replace function public.set_wrenchready_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists wrenchready_inbound_set_updated_at on public.wrenchready_inbound;
create trigger wrenchready_inbound_set_updated_at
before update on public.wrenchready_inbound
for each row
execute function public.set_wrenchready_updated_at();
