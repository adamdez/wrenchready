...
| Hard-coded FAL key in helper scripts | implemented in working tree, rotation blocked | `poll-rotor.py`; `composite-and-submit.py`; `gen-rotor-clip.py` | Removed hard-coded key and require `FAL_KEY` from environment | Rotate the exposed key and purge/accept git-history exposure; evaluation docs still quote the leaked key as evidence |
