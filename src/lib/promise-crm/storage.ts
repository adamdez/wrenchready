...
export type PromiseQuoteDraftForReviewInput = {
  promiseId?: string;
  customerName: string;
  customerPhone?: string;
  customerEmail?: string;
  preferredContact?: PromiseRecord["customer"]["preferredContact"];
  vehicleLabel: string;
  address?: string;
  city?: string;
  territory?: string;
...
function normalizeQuoteDraftPreferredContact(
  value?: PromiseRecord["customer"]["preferredContact"],
): PromiseRecord["customer"]["preferredContact"] {
  return value === "text" || value === "email" || value === "call" ? value : "call";
}

function inferQuoteDraftLocation(input: PromiseQuoteDraftForReviewInput) {
  const normalizedAddress = (input.address || "").toLowerCase();
  const city =
    input.city?.trim() ||
    (normalizedAddress.includes("spokane valley")
      ? "Spokane Valley"
      : normalizedAddress.includes("liberty lake")
        ? "Liberty Lake"
        : "Spokane");
  const territory =
    input.territory?.trim() ||
    (city === "Spokane Valley" ? "Spokane Valley" : city === "Liberty Lake" ? "Liberty Lake" : "Spokane");

  return {
    label: input.address?.trim() || "Address pending",
    city,
    territory,
...
function quoteDraftTopRisks(input: PromiseQuoteDraftForReviewInput) {
  const risks = new Set(
    (input.topRisks || [])
      .map((risk) => risk.trim())
      .filter(Boolean),
  ...
  risks.add("Human review is required before this quote is sent to the customer.");
  risks.add("No payment link or checkout request is generated by the quote draft workflow.");
  if (input.quoteAmount === undefined) {
    risks.add("Quote amount still needs office confirmation.");
  }
  if (!input.scheduledWindowLabel?.trim()) {
    risks.add("Schedule window still needs office confirmation.");
  }

  return [...risks];
}

function buildQuoteDraftFieldExecution(
  input: PromiseQuoteDraftForReviewInput,
): PromiseFieldExecutionPacket {
  return {
    serviceGoal: input.fieldExecution?.serviceGoal || input.serviceScope,
    partsChecklist:
      input.fieldExecution?.partsChecklist && input.fieldExecution.partsChecklist.length > 0
        ? input.fieldExecution.partsChecklist
    ...
    photosRequired:
      input.fieldExecution?.photosRequired && input.fieldExecution.photosRequired.length > 0
        ...
        : ["Capture proof photos or screenshots that support the diagnostic conclusion."],
    inspectionChecklist:
    ...
    handoffChecklist:
      input.fieldExecution?.handoffChecklist && input.fieldExecution.handoffChecklist.length > 0
        ...
        : [
            "Review quote amount, scope, caveats, and schedule window before customer send.",
            "Tell the customer this quote does not authorize parts or extra repair time unless listed.",
          ],
    comebackPreventionSteps:
      input.fieldExecution?.comebackPreventionSteps && input.fieldExecution.comebackPreventionSteps.length > 0
        ? input.fieldExecution.comebackPreventionSteps
        : ["State what is proven versus still suspected before recommending parts."],
    notesTemplate: input.fieldExecution?.notesTemplate,
    upsellFocus: input.fieldExecution?.upsellFocus || [],
    closeoutSteps:
      input.fieldExecution?.closeoutSteps && input.fieldExecution.closeoutSteps.length > 0
        ? input.fieldExecution.closeoutSteps
        : ["Save final test evidence.", "Prepare customer recap before requesting payment."],
  };
}

function buildQuoteDraftVisibleNotes(input: PromiseQuoteDraftForReviewInput) {
  return [
    `Quote draft prepared for human review${input.sourceLabel ? ` from ${input.sourceLabel}` : ""}.`,
    input.quoteSummary ? `Quote summary: ${input.quoteSummary}` : undefined,
    "Human review required before customer send.",
    "Payment link intentionally not generated by this workflow.",
    ...(input.notes || []),
...
function buildQuoteDraftPaymentCollection(): PromisePaymentCollection {
  return {
    ...
    invoiceReference: "Draft quote - checkout not generated",
    paymentSummary:
      "Payment link intentionally not generated until human review approves sending the quote.",
  };
}

export async function upsertPromiseQuoteDraftForReview(
  input: PromiseQuoteDraftForReviewInput,
) {
  ...
  const customerMessage =
    ...
    [
      ...
      "This does not authorize parts, extra repair time, or additional work unless WrenchReady explains it and gets approval first.",
    ].filter(Boolean).join(" ");
  const economics = normalizePromiseEconomics({
    quotedAmount: input.quoteAmount,
    laborHours: input.laborHours,
    partsCostAmount: input.partsCostAmount ?? 0,
    cardFeePercent: 3,
    warrantyReservePercent: 2,
  });
  const customerApproval = ensurePromiseCustomerApproval({
    status: "awaiting-approval",
    requestedAt: now,
    requestedService: input.serviceScope,
    requestedAmount: input.quoteAmount,
    summary: quoteSummary,
    customerMessage,
  });
  const paymentCollection = buildQuoteDraftPaymentCollection();
  const fieldExecution = buildQuoteDraftFieldExecution(input);
  const scheduledWindowLabel = input.scheduledWindowLabel?.trim() || "Needs office scheduling review";
  const readinessSummary =
    input.readinessSummary ||
    "Quote draft is ready for Adam/Dez review before customer send, schedule confirmation, or payment link creation.";
  const nextAction =
    input.nextAction ||
    "Adam/Dez review quote scope, amount, caveats, and schedule window before sending to the customer.";
  const topRisks = quoteDraftTopRisks(input);

  if (input.promiseId) {
    return updatePromiseRecord(input.promiseId, {
      owner: input.owner || "Dez",
      readinessRisk: input.readinessRisk || "medium",
      ...
      nextAction,
      topRisks,
      noteToAdd: `Jeff quote draft prepared for human review: ${quoteSummary}`,
      economics,
      customerApproval,
      customerCertainty: {
        contactConfirmed: Boolean(input.customerPhone),
      ...
      dayReadiness: {
        ...
        locationConfirmed: Boolean(input.address),
  ...
  const customerAccess = createPromiseCustomerAccess();
  const visibleNotes = buildQuoteDraftVisibleNotes(input);
  const notes = mergePromiseNotesWithExecutionOps(
    mergePromiseNotesWithReadinessState(
      mergePromiseNotesWithCustomerState(
        mergePromiseNotesWithEconomics(visibleNotes, economics),
        customerAccess,
        customerApproval,
      ),
      {
  ...
  const promiseRow: Omit<SupabasePromiseRow, "id" | "created_at" | "updated_at"> = {
    ...
    preferred_contact: normalizeQuoteDraftPreferredContact(input.preferredContact),
    vehicle_year: vehicle.year,
    vehicle_make: vehicle.make,
    vehicle_model: vehicle.model,
    vehicle_mileage: null,
    ...
    service_scope: input.serviceScope,
    owner: input.owner || "Dez",
    readiness_risk: input.readinessRisk || "medium",
    status: "promises-waiting",
    scheduled_window_label: scheduledWindowLabel,
    scheduled_window_start: input.scheduledWindowStartIso || null,
    scheduled_window_end: input.scheduledWindowEndIso || null,
  ...
  if (!hasPromiseCrmSupabase()) {
    if (!demoFallbackEnabled()) {
      throw crmUnavailable(
        "Promise CRM quote draft create unavailable: Supabase URL or server credential is missing.",
      );
    }
    const created = mapPromiseRow({
      id: crypto.randomUUID(),
      created_at: now,
      updated_at: now,
      ...promiseRow,
  ...
  try {
    const rows = await supabaseRestRequest<SupabasePromiseRow[]>("wrenchready_promise", {
      method: "POST",
      body: promiseRow,
    });

    if (!rows[0]) throw new Error("Supabase did not return the created quote draft.");
    return mapPromiseRow(rows[0]);
  } catch (error) {
    if (!demoFallbackEnabled()) {
      throw crmUnavailable("Promise CRM quote draft create failed.", error);
    }
    const created = mapPromiseRow({
      id: crypto.randomUUID(),
      created_at: now,
